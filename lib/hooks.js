...
function getDescriptorFromUserAgent(ua) {
  ...
  UA.meetyou = getDescriptorFromMeetYouClient(ua);
  UA.wechat = getDescriptorFromWechat(ua);
  return UA;
}

function getDescriptorFromMeetYouClient(ua) {
  var m = ua.match(/MeetYouClient\/([\d.]*)([\s]+)([\(]+)([\d]*)([\)+])/);
  if (m) {
    return {
...
function getDescriptorFromWechat(ua) {
...
UA.getDescriptorFromMeetYouClient = getDescriptorFromMeetYouClient;
UA.getDescriptorFromWechat = getDescriptorFromWechat;
...
if (documentElement) {
  for (var i = 0; i < browsers.length; i++) {
    ...
    var v = UA[key];
    if (v) {
      className += ' meetyou-' + key + (parseInt(v, 10) + '');
      className += ' meetyou-' + key;
    }
  }
  if (className) {
